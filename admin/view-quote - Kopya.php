...
<head>
    ...
    <style>
        * {
        ...
        .transport-process-content .editable {
            display: block;
            min-height: 20px;
            pointer-events: auto;
        }
        ...
        .editable-transport-process:hover {
            border-color: #007bff;
            background-color: rgba(0,123,255,0.05);
        }

        .editable-transport-process:empty:before {
            content: "Taşınma sürecini düzenlemek için buraya tıklayın...";
            color: #6c757d;
            font-style: italic;
        }
            ...
            <div class="details-side">
                ...
                <div class="form-section">
                    ...
                    <div class="form-content">
                        ...
                        <?php if (stripos($quote['transport_name'], 'deniz') !== false): ?>
                        <div class="form-row">
                            <span class="form-label editable" data-field="container_type_label" data-quote-id="<?php echo $quote['quote_number']; ?>">
                                Konteyner Tipi
                                <span class="edit-indicator"></span>
                            </span>
                            <span class="form-value editable" data-field="container_type" data-quote-id="<?php echo $quote['quote_number']; ?>" data-type="select" data-options='{"":"Seçiniz","20FT":"20 FT Konteyner","40FT":"40 FT Konteyner","40FT_HC":"40 FT HC Konteyner"}'>
                                <?php
                                $container_types = [
                                ...
                                echo !empty($quote['container_type']) ? ($container_types[$quote['container_type']] ?? $quote['container_type']) : 'Seçiniz';
                                ?>
                                <span class="edit-indicator"></span>
                            </span>
                        </div>
                ...
                <div class="form-section">
                    ...
                    <div class="form-content">
                        ...
                        <div class="form-row" data-field="unit_price" style="<?php echo empty($quote['unit_price']) ? 'display: none;' : ''; ?>">
                        ...
                        <div class="form-row" data-field="pieces" style="<?php echo empty($quote['pieces']) ? 'display: none;' : ''; ?>">
                        ...
                        <div class="form-row" data-field="cargo_type" style="<?php echo empty($quote['cargo_type']) ? 'display: none;' : ''; ?>">
                            <span class="form-label editable" data-field="cargo_type_label" data-quote-id="<?php echo $quote['quote_number']; ?>">
                                <?php echo $t['cargo_type']; ?>
                                <span class="edit-indicator"></span>
                            </span>
                        ...
                        <div class="form-row" data-field="trade_type" style="<?php echo empty($quote['trade_type']) ? 'display: none;' : ''; ?>">
                            <span class="form-label editable" data-field="trade_type_label" data-quote-id="<?php echo $quote['quote_number']; ?>">
                                <?php echo $t['trade_type']; ?>
                                <span class="edit-indicator"></span>
                            </span>
                        ...
                        <div class="form-row" data-field="description" style="<?php echo empty($quote['description']) ? 'display: none;' : ''; ?>">
            ...
            <div class="info-side">
                ...
                <div class="form-section">
                    ...
                    <div class="form-content">
                        ...
                        // Öncelikle veritabanında teklif satırında özel bir içerik var mı kontrol et
...
8-Eşyanın teslimat adresine teslimi şeklindedir.";
                        ...
                        <div class="transport-process-content">
                            <div class="editable-transport-process editable" data-field="transport_process_text" data-quote-id="<?php echo $quote['quote_number']; ?>" data-type="textarea">
                ...
                <!-- Referans Görseller Ayarları -->
                <div class="form-section">
                    <div class="section-header">
                        <div class="section-label">
                            <i class="fas fa-images"></i> Referans Görseller
                        </div>
                        <div class="section-title"></div>
                    </div>
                    <div class="form-content">
                        <div class="form-group mb-3">
                            <label class="form-label">Müşteri Görünümünde Gösterim</label>
                            <div class="form-check form-switch">
                                <input class="form-check-input" type="checkbox" id="showReferenceImages"
                                       <?= ($quote['show_reference_images'] ?? 1) ? 'checked' : '' ?>
                                       onchange="updateReferenceImagesVisibility(this.checked, '<?= $quote['quote_number'] ?>')">
                                <label class="form-check-label" for="showReferenceImages">
                                    Referans görselleri müşteri görünümünde göster
                                </label>
                            </div>
                            <small class="form-text text-muted">
                                Bu seçenek kapalıysa, müşteri teklif sayfasında referans görseller bölümü görünmez.
                            </small>
                        </div>

                        <div class="alert alert-info">
                            <i class="fas fa-info-circle"></i>
                            <strong>Bilgi:</strong> Referans görselleri admin panelinden
                            <a href="transport-modes.php" target="_blank">Taşıma Modları</a> bölümünden yönetebilirsiniz.
                        </div>
                    </div>
                </div>
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            editables.forEach(editable => {
                editable.addEventListener('click', function(e) {
                    console.log('Editable clicked:', this.dataset.field);
        ...
        function startEditing(element) {
            ...
            controls.innerHTML = `
                <button type="button" class="btn-save" title="Kaydet">
                    <i class="fas fa-check"></i>
                </button>
                <button type="button" class="btn-cancel" title="İptal">
                    <i class="fas fa-times"></i>
                </button>
            `;
        ...
        function saveEdit(element, input, originalValue, field, quoteId) {
            ...
            .then(data => {
                if (data.success) {
                    ...
                    // Konteyner tipi güncellendiğinde başarı mesajı göster
                    if (field === 'container_type') {
                        showAlert('Konteyner tipi başarıyla güncellendi!', 'success');
                    }

                    // Taşıma modu değiştirildiğinde konteyner tipi alanını güncelle
                    if (field === 'custom_transport_name') {
                        setTimeout(() => {
                            toggleContainerTypeField();
                        }, 500);
                    }
                ...
                <div class="mb-3">
                    <div class="btn-toolbar mb-2" role="toolbar">
                        ...
                        <div class="btn-group ms-2" role="group">
                            <input type="file" id="richFileInput" style="display:none;" onchange="uploadEditorFile(this)">
                            <button type="button" class="btn btn-sm btn-primary" onclick="document.getElementById('richFileInput').click()" title="Dosya yükleyip içeriğe link olarak ekle">
                                <i class="fas fa-upload me-1"></i>
                                <span class="d-none d-md-inline">Dosya Yükle</span>
                            </button>
                        </div>
                    </div>
                </div>
    ...
    <script>
        ...
        function uploadEditorFile(input) {
            ...
            .then(data => {
                ...
                if (data.success && data.url) {
                    // İçeriğe link ekle
                    document.execCommand('insertHTML', false, `<a href=\"${data.url}\" target=\"_blank\">${file.name}</a>`);
                } else {
        ...
        // Transport mode değiştirildiğinde konteyner tipi alanını göster/gizle
        function toggleContainerTypeField() {
            const customTransportElement = document.querySelector('[data-field="custom_transport_name"]');
            const containerTypeRow = document.querySelector('[data-field="container_type"]')?.closest('.form-row');

            if (customTransportElement && containerTypeRow) {
                const transportName = customTransportElement.textContent.trim().toLowerCase();

                if (transportName.includes('deniz') || transportName.includes('sea')) {
                    containerTypeRow.style.display = 'flex';
                } else {
                    containerTypeRow.style.display = 'none';
                }
            }
        }

        // Referans görseller görünürlüğünü güncelle
        function updateReferenceImagesVisibility(isVisible, quoteNumber) {
            ...
            fetch(window.location.href, {
                ...
                body: `ajax=1&action=update_field&field=show_reference_images&value=${value}&quote_number=${quoteNumber}`
            })
            .then(response => {
                if (!response.ok) {
            ...
            .then(data => {
                if (data.success) {
                    ...
                    toast.innerHTML = `
                        ...
                        Referans görseller ayarı güncellendi
                    ...
                    document.getElementById('showReferenceImages').checked = !isVisible;
                }
            })
            .catch(error => {
                console.error('Error:', error);
        ...
        // Sayfa yüklendiğinde konteyner tipi alanını kontrol et
        document.addEventListener('DOMContentLoaded', function() {
            toggleContainerTypeField();
        });
    </script>
